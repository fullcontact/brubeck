...
static void try_parse(struct brubeck_statsd_msg *msg, const char *msg_text, double expected, float sample_rate)
{
	...
	sput_fail_unless(fabs(sample_rate - msg->sample_rate) < 1e-6, "msg.sample_rate == sample_rate");
}
...
// TODO(xorlev) support this	try_parse(&msg, "github.auth.fingerprint.sha1:1|c|@args", 1, 1.0);
